...
static int isotp_send_flow_control(IsoTpLink* link, uint8_t flow_status, uint8_t block_size, uint8_t st_min_ms) {
	...
	ret = link->send_callback(link->send_arbitration_id, message.as.data_array.ptr, sizeof(message));
#else    
	ret = link->send_callback(link->send_arbitration_id,
...
static int isotp_send_single_frame(IsoTpLink* link, uint32_t id) {
	...
	ret = link->send_callback(id, message.as.data_array.ptr, sizeof(message));
#else
	ret = link->send_callback(id,
            message.as.data_array.ptr,
            link->send_size + 1);
#endif
...
int isotp_send_poll(IsoTpLink *const _link, const uint8_t *const _buf, const uint16_t _size)
{
	...
	{
		if (0 == (size = _link->receive_callback(&id, buf, sizeof(buf))) || _link->receive_arbitration_id != id)
			continue;
...
int isotp_send_with_id(IsoTpLink *link, uint32_t id, const uint8_t payload[], uint16_t size) {
    ...
    if (link->send_size < 8) {
        ...
        if (ISOTP_RET_OK == ret) {
            ...
            link->send_timer_st = link->sys_time_callback();
            link->send_timer_bs = link->sys_time_callback() + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
...
void isotp_indication(IsoTpLink *link, const uint8_t *const data, const uint8_t len) {
    IsoTpCanMessage message;
    int ret;
    ...
    switch (message.as.common.type) {
        case ISOTP_PCI_TYPE_SINGLE: {
            ...
            if (ISOTP_RET_OK == ret) {
				...
				link->indication_callback(link, link->receive_buffer, link->receive_size);
            }
            break;
        }
        case ISOTP_PCI_TYPE_FIRST_FRAME: {
            ...
            if (ISOTP_RET_OK == ret) {
                ...
                /* refresh timer cs */
                link->receive_timer_cr = link->sys_time_callback() + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
            }
...
void isotp_init_link(IsoTpLink *link, uint32_t sendid, uint32_t recvid,
	                 ...
	                 sys_time_callback_t sys_time_callback,
					 debug_callback_t debug_callback)
{
	...
	link->sys_time_callback = sys_time_callback;
...
void isotp_poll(IsoTpLink *link) {
    ...
    if (ISOTP_SEND_STATUS_INPROGRESS == link->send_status) {
        ...
        (0 == link->send_st_min || (0 != link->send_st_min && IsoTpTimeAfter(link->sys_time_callback(), link->send_timer_st)))) {
            ...
            if (ISOTP_RET_OK == ret) {
                ...
                link->send_timer_bs = link->sys_time_callback() + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
                link->send_timer_st = link->sys_time_callback() + link->send_st_min;
        ...
        /* check timeout */
        if (IsoTpTimeAfter(link->sys_time_callback(), link->send_timer_bs)) {
    ...
    if (ISOTP_RECEIVE_STATUS_INPROGRESS == link->receive_status) {
        ...
        /* check timeout */
        if (IsoTpTimeAfter(link->sys_time_callback(), link->receive_timer_cr)) {
